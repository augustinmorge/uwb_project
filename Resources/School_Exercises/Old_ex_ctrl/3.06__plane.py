...
def control(x):
    x = x.flatten()
    phi, theta, psy = x[3], x[4], x[5]
    
    # Régulateur haut niveau
    theta_bar = -0.2*arctan((zbar-x[2])/10)  # asiette voulue
    p = array([[x[0]], [x[1]]])
    psy_bar = arctan2(x[1], x[0])+pi/2+arctan((norm(p)-rbar)/50)  # cap voulu

    # Régulateur bas niveau
    v = x[6:9].reshape(3, 1)
    
    # Régulation en vitesse
    u1 = 5*(1+2/pi*arctan(vbar - norm(v)))
    
    # Régulation en altitude
    u2 = -0.3*(2/pi*arctan(5*(theta_bar-theta))+abs(sin(phi)))
    
    # Régulation en cap
    phi_bar = 0.5*arctan(5*sawtooth(psy_bar-psy))
    u3 = -0.3*2/pi*arctan(phi_bar-phi)
...
fig   = figure()
ax    = Axes3D(fig)
plot3D(ax, array([Cx0, Cy0, Cz0]).T, 'green')

...
def plot_data(ids, time, dist, RX, with_RX, FP, Q, with_all, sigma_rw,display_allan=1, display_dbm=1, display_quality=1, filename = None):
    ...
    if display_dbm:
        if with_all:
            ...
            fig.suptitle(f"Range for anchor {int(ids[0]) - 1780}")
        
            ax.scatter(time/60/60, dist, s=0.1,color='black')
            ax.set_xlabel("Time [h]")
            ax.set_ylabel("m")
    ...
    if display_allan:
        ...
        ax.set_xlabel('h')
        ax.set_ylabel('m')
        ax.set_title(f'Allan Deviation for anchor {int(ids[0]) - 1780}') # for anchor {}'.format(int(ids[0])))
...
if __name__ == "__main__":
    ...
    # filenames = [os.path.join(THIS_FOLDER, "Long_log_07_04_2023_17_09_02.csv")] 

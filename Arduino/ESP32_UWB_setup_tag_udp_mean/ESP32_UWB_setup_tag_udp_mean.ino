...
void newRange()
{
    ...
    fresh_link(uwb_data, DW1000Ranging.getDistantDevice()->getShortAddress(), DW1000Ranging.getDistantDevice()->getRange(), \
                         DW1000Ranging.getDistantDevice()->getRXPower(), DW1000.getFirstPathPower(), DW1000.getReceiveQuality(), timer); //DW1000.getFirstPathPower() //DW1000Ranging.getDistantDevice()->getRXPower()
...
void fresh_link(struct Link *p, uint16_t addr, float range, float dbm, float fp, float quality, long unsigned int timer)
{
// #ifdef DEBUG
...
void print_link(struct Link *p)
{
    ...
    while (temp->next != NULL)
    {
        ...
        Serial.println(temp->next->timer);
        temp = temp->next;
    }
...
void make_link_json(struct Link *p, String *s)
{
    ...
    *s = "{\"links\":[";
    ...
    while (temp->next != NULL)
    {
        ...
        char link_json[120];
        sprintf(link_json, "{\"A\":\"%X\",\"R\":\"%.3f\",\"RX\":\"%.3f\",\"FP\":\"%.3f\",\"Q\":\"%.3f\",\"T\":\"%lu\"}", temp->anchor_addr, temp->range[0], temp->dbm, temp->fp, temp->quality, temp->timer);
        *s += link_json;
        if (temp->next != NULL)
        {
            *s += ",";
